 ...
 * ftarget   : target function value; optimization stops when f <= ftarget for a
 * feasible point, can be set to -INFINITY to disable maxfun    : maximum number
 * of function evaluations npt       : number of points in the interpolation
 * set, n+2<=npt<=(n+1)(n+2)/2, recommended: 2*n+1 iprint    : verbosity level,
 * see the prima_message enum m_nlcon   : number of non-linear constraints (>=0)
 ...
 *             Aineq is an m_ineq-by-n matrix stored in row-major order (line by
 * line) bineq is of size m_ineq m_eq, Aeq, beq : Aeq*x = beq constraint Aeq is
 * an m_eq-by-n matrix stored in row-major order (line by line) beq is of size
 * m_eq xl, xu    : x lower & upper bounds, of size n
 *
 * return    : see prima_rc enum for return codes
 */

PRIMAC_API
int prima_bobyqa(const prima_obj calfun, const void *data, const int n,
                 double x[], double *f, const double xl[], const double xu[],
                 int *nf, const double rhobeg, const double rhoend,
                 const double ftarget, const int maxfun, const int npt,
                 const int iprint);

PRIMAC_API
int prima_newuoa(const prima_obj calfun, const void *data, const int n,
                 double x[], double *f, int *nf, const double rhobeg,
                 const double rhoend, const double ftarget, const int maxfun,
                 const int npt, const int iprint);

PRIMAC_API
int prima_uobyqa(const prima_obj calfun, const void *data, const int n,
                 double x[], double *f, int *nf, const double rhobeg,
                 const double rhoend, const double ftarget, const int maxfun,
                 const int iprint);

PRIMAC_API
int prima_cobyla(const int m_nlcon, const prima_objcon calcfc, const void *data,
                 const int n, double x[], double *f, double *cstrv,
                 double nlconstr[], const int m_ineq, const double Aineq[],
                 const double bineq[], const int m_eq, const double Aeq[],
                 const double beq[], const double xl[], const double xu[],
                 int *nf, const double rhobeg, const double rhoend,
                 const double ftarget, const int maxfun, const int iprint);

PRIMAC_API
int prima_lincoa(const prima_obj calfun, const void *data, const int n,
                 double x[], double *f, double *cstrv, const int m_ineq,
                 const double Aineq[], const double bineq[], const int m_eq,
                 const double Aeq[], const double beq[], const double xl[],
                 const double xu[], int *nf, const double rhobeg,
                 const double rhoend, const double ftarget, const int maxfun,
                 const int npt, const int iprint);
